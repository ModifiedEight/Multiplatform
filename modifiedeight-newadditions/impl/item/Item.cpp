...
void Item::initItems(std::shared_ptr<TextureAtlas> a2){
	...
	Item::sword_gold = (new WeaponItem(27, Item::Tier::GOLD))->setIcon("sword", 3)->setCategory(3, 2)->setDescriptionId("swordGold");
	...
	Item::bed = (new BedItem(99))->setMaxStackSize(1)->setIcon("bed", 0)->setCategory(5, 1)->setDescriptionId("bed");
...
bool_t Item::useOn(struct ItemInstance*, struct Player* player, Level* level, int32_t x, int32_t y, int32_t z, int32_t side, float hitX, float hitY, float hitZ) {
	if(this == Item::paper) {
		...
		if (Tile::mixedSlab && tileId == Tile::mixedSlab->blockID) {
			...
			if (te) {
				...
				if (te->mode == 1) {
					...
					else if (side == 2) hitTop = false;
					else if (side == 3) hitTop = true;
					else hitTop = (hitZ >= 0.5f);
				} else if (te->mode == 2) {
					if (te->bottomTileId != 0 && te->topTileId == 0) hitTop = false;
					else if (te->topTileId != 0 && te->bottomTileId == 0) hitTop = true;
					else if (side == 4) hitTop = false;
					else if (side == 5) hitTop = true;
					else hitTop = (hitX >= 0.5f);
				} else {
					if (te->bottomTileId != 0 && te->topTileId == 0) hitTop = false;
					else if (te->topTileId != 0 && te->bottomTileId == 0) hitTop = true;
					else if (side == 0) hitTop = false;
					else if (side == 1) hitTop = true;
					else hitTop = (hitY >= 0.5f);
				}

				if (hitTop) {
					te->topColor = 0xFFFFFF;
				} else {
		...
		if(tileId == Tile::cloth->blockID || tileId == Tile::woolCarpet->blockID) {
			level->setTileAndData(x, y, z, tileId, 0, 3);
			return 1;
		}
		if(Tile::stainedGlass && tileId == Tile::stainedGlass->blockID) {
			level->setTileAndData(x, y, z, Tile::glass->blockID, 0, 3);
			return 1;
		}
		...
		int32_t tileId = level->getTile(x, y, z);
		if(tileId == Tile::grass->blockID && side == 1) { // 1 is TOP
			level->setTileAndData(x, y, z, Tile::grassPath->blockID, 0, 3);
			level->playSound((float)x + 0.5f, (float)y + 0.5f, (float)z + 0.5f, "step.grass", 1.0f, 0.8f);
			if(player) {
				// We don't have hurtAndBreak here, but we can just let it be or just do a simple damage.
				// For MCPE 0.8.1, item damage is usually handled in Item::mineBlock or we can call hurtAndBreak? 
				// The original code for hoe uses useOn. Let's just return true.
			}
			return 1;
		}
	}
