...
LABEL_16:
	...
	if(!v46->options.field_F1) {
		...
		if(Tile::isBed(v12->getTile(v13, v14, v15))) {
...
static float _getRenderDistance(int dist) {
    if (dist <= -4) return 768.0f;
    if (dist == -3) return 512.0f;
    if (dist == -2) return 384.0f;
    if (dist == -1) return 256.0f;
    if (dist == 0) return 192.0f;
    if (dist == 1) return 128.0f;
    if (dist == 2) return 64.0f;
    if (dist >= 3) return 32.0f;
    return 192.0f;
}
...
void GameRenderer::setupCamera(float a2, int32_t a3) {
	...
	minecraft = this->minecraft;
	v6 = _getRenderDistance(minecraft->options.renderDistance);
	this->field_8 = v6;
...
void GameRenderer::setupFog(int32_t a2) {
	...
	if (Options::instance && Options::instance->fogEnabled == 0) {
#ifdef USEGLES
		glFogx(GL_FOG_MODE, GL_LINEAR);
#else
		glFogi(GL_FOG_MODE, GL_LINEAR);
#endif
		glFogf(GL_FOG_START, this->field_8 * 1.5f);
		glFogf(GL_FOG_END, this->field_8 * 2.0f);
		glEnable(GL_FOG);
		this->field_150 = 0;
		return;
	}
