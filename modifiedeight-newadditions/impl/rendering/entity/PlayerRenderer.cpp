...
static bool isPlayerSwimming(Player* p) {
	if (!p || !p->isAlive() || p->isSleeping() || p->isRiding()) return false;
	if (p->entityHeight < 0.8f) return true;
	if (p->isInWater() || p->isUnderLiquid(Material::water)) return true;
	if (p->level) {
		int px = (int)floorf(p->posX);
		int pz = (int)floorf(p->posZ);
		for (float yo = -0.5f; yo <= 1.4f; yo += 0.35f) {
			int py = (int)floorf(p->posY + yo);
			if (py >= 0 && py < 128) {
				int tile = p->level->getTile(px, py, pz);
				Material* mat = p->level->getMaterial(px, py, pz);
				if (tile == Tile::water->blockID || tile == Tile::calmWater->blockID || (Tile::seagrass && tile == Tile::seagrass->blockID) || (mat && mat == Material::water)) {
					return true;
				}
			}
		}
