...
void ExternalFileLevelStorageSource::renameLevel(const std::string& levelId, const std::string& newName) {
	std::string path = this->getFullPath(levelId);
	LevelData ld;
	if(ExternalFileLevelStorage::readLevelData(path, ld)) {
		ld.setLevelName(newName);
		ExternalFileLevelStorage::saveLevelData(path, ld, 0);
	}
}
