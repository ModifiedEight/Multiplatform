...
void ExternalFileLevelStorage::tick() {
	if(this->level) {
		...
		if((this->field_24 % 50) == 0) {
			ChunkCache* chunkCache = (ChunkCache*)this->level->chunkSource;
			if(chunkCache) {
				for(int32_t i = 0; i < 4096; ++i) {
					LevelChunk* chunk = chunkCache->chunks[i];
					if(chunk && chunk != chunkCache->emptyChunk && chunk->unsaved) {
						int32_t index = (chunk->chunkX & 31) + 32 * (chunk->chunkZ & 31);
						for(auto&& el: this->field_2C) {
							if(el.chunk->chunkX == chunk->chunkX && el.chunk->chunkZ == chunk->chunkZ) {
								el.timeMs = RakNet::GetTimeMS();
								goto LABEL_12;
							}
						}

						this->field_2C.push_back({index, (int32_t)RakNet::GetTimeMS(), chunk});
LABEL_12:
						chunk->unsaved = 0;
					}
				}
