...
static JNIEnv *getEnv() {
  AppPlatform_android *platform =
      (AppPlatform_android *)AppPlatform::_singleton;
  if (!platform)
    return nullptr;
  JNIEnv *env;
...
CrossPlatform_DownloadBinary(const std::string &rawUrl) {
  ...
  jmethodID getInputStream =
      env->GetMethodID(connClass, "getInputStream", "()Ljava/io/InputStream;");
...
bool CrossPlatform_ExtractZip(const std::string &zipFile,
  ...
  jmethodID fileGetParentFile =
      env->GetMethodID(fileClass, "getParentFile", "()Ljava/io/File;");

  jmethodID fisInit =
      env->GetMethodID(fisClass, "<init>", "(Ljava/lang/String;)V");
  jmethodID zisInit =
      env->GetMethodID(zisClass, "<init>", "(Ljava/io/InputStream;)V");
  jmethodID getNextEntry =
      env->GetMethodID(zisClass, "getNextEntry", "()Ljava/util/zip/ZipEntry;");
  jmethodID isDirectory = env->GetMethodID(zeClass, "isDirectory", "()Z");
  jmethodID getName =
      env->GetMethodID(zeClass, "getName", "()Ljava/lang/String;");
  ...
  jmethodID fosInit =
      env->GetMethodID(fosClass, "<init>", "(Ljava/lang/String;)V");
...
typedef HINTERNET(WINAPI *InternetOpenA_t)(LPCSTR, DWORD, LPCSTR, LPCSTR,
                                           DWORD);
typedef HINTERNET(WINAPI *InternetOpenUrlA_t)(HINTERNET, LPCSTR, LPCSTR, DWORD,
                                              DWORD, DWORD_PTR);
...
CrossPlatform_DownloadBinary(const std::string &rawUrl) {
  ...
  InternetOpenA_t pInternetOpenA =
      (InternetOpenA_t)GetProcAddress(hWinInet, "InternetOpenA");
  InternetOpenUrlA_t pInternetOpenUrlA =
      (InternetOpenUrlA_t)GetProcAddress(hWinInet, "InternetOpenUrlA");
  InternetReadFile_t pInternetReadFile =
      (InternetReadFile_t)GetProcAddress(hWinInet, "InternetReadFile");
  InternetCloseHandle_t pInternetCloseHandle =
      (InternetCloseHandle_t)GetProcAddress(hWinInet, "InternetCloseHandle");

  if (!pInternetOpenA || !pInternetOpenUrlA || !pInternetReadFile ||
      !pInternetCloseHandle)
    return {};

  HINTERNET hInternet = pInternetOpenA("MinecraftPE_DL", 0, NULL, NULL,
                                       0); // INTERNET_OPEN_TYPE_PRECONFIG = 0
  if (!hInternet)
    return {};
  HINTERNET hUrl = pInternetOpenUrlA(
      hInternet, url.c_str(), NULL, 0, 0x80000000 | 0x04000000,
      0); // INTERNET_FLAG_RELOAD | INTERNET_FLAG_SECURE
  if (!hUrl) {
  ...
  char buffer[8192];
  DWORD bytesRead;
  while (pInternetReadFile(hUrl, buffer, sizeof(buffer), &bytesRead) &&
         bytesRead > 0) {
...
bool CrossPlatform_ExtractZip(const std::string &zipFile,
                              const std::string &outDir) {
  std::string cmd = "mkdir -p \"" + outDir + "\" && unzip -o \"" + zipFile +
                    "\" -d \"" + outDir + "\" >/dev/null 2>&1";
  int res = system(cmd.c_str());
  int exit_code = 0;
#ifdef WEXITSTATUS
