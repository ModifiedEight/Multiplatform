...
bool_t Player::checkBed() {
	return Tile::isBed(this->level->getTile(this->bedPosition.x, this->bedPosition.y, this->bedPosition.z));
}
...
void Player::tick() {
	...
	if(v3 != this->isSleeping()) {
		printf("[DEBUG] BedSync mismatch: v3 (synced flag) = %d, isSleeping() = %d\n", v3, this->isSleeping());
		if(this->isSleeping()) {
			printf("[DEBUG] Waking up because sync flag is 0\n");
			...
			printf("[DEBUG] Sleeping because sync flag is 1 at %d,%d,%d\n", v14.x, y, z);
	...
	if(this->isSleeping()) {
		...
		if(!level->isClientMaybe) {
			bool_t hasBed = this->checkBed();
			bool_t isDaytime = this->level->isDay();
			if(hasBed) {
				if(isDaytime) {
					printf("[DEBUG] Waking up because isDay() is true\n");
					this->stopSleepInBed(0, 1, 1);
				}
			} else {
				printf("[DEBUG] Waking up because checkBed() is false (getTile at %d,%d,%d returned %d)\n",
					this->bedPosition.x, this->bedPosition.y, this->bedPosition.z,
					this->level->getTile(this->bedPosition.x, this->bedPosition.y, this->bedPosition.z));
...
int32_t Player::startSleepInBed(int32_t x, int32_t y, int32_t z) {
	...
	printf("[DEBUG] startSleepInBed called at %d,%d,%d. isClientMaybe = %d\n", x, y, z, this->level->isClientMaybe);
	if(!this->level->isClientMaybe) {
		if(this->isSleeping() || !this->isAlive()) {
			printf("[DEBUG] startSleepInBed failed: isSleeping = %d, isAlive = %d\n", this->isSleeping(), this->isAlive());
			return 4;
		}
		v19 = (float)x;
		if(fabsf(this->posX - (float)x) > 3.0 || fabsf(this->posY - (float)y) > 4.0 || fabsf(this->posZ - (float)z) > 3.0) {
			printf("[DEBUG] startSleepInBed failed: too far away (player: %f,%f,%f bed: %d,%d,%d)\n", this->posX, this->posY, this->posZ, x, y, z);
			return 3;
		}
		bool_t natural = this->level->dimensionPtr->isNaturalDimension();
		printf("[DEBUG] dimension isNaturalDimension = %d\n", natural);
		if(natural) {
			printf("[DEBUG] startSleepInBed failed: not a natural dimension (isNaturalDimension returned true)\n");
			return 1;
		}
		if(this->level->isDay()) {
			printf("[DEBUG] startSleepInBed failed: isDay() is true\n");
			return 2;
		}

		std::vector<struct Entity*> v20;																							  // [sp+4h] [bp-4Ch] BYREF
		AABB v21{.minX = v19 - 8.0f, .minY = (float)y - 5.0f, .minZ = (float)z - 8.0f, .maxX = v19 + 8.0f, .maxY = (float)y + 5.0f, .maxZ = (float)z + 8.0f}; // [sp+10h] [bp-40h] BYREF
		level = this->level;
		level->getEntitiesOfClass(1, v21, v20);
		if(!v20.empty()) {
			printf("[DEBUG] startSleepInBed failed: monsters nearby (%zu monsters)\n", v20.size());
			return 5;
		}
	}
	printf("[DEBUG] startSleepInBed succeeded, setting player state to sleeping\n");
	this->setSize(0.2, 0.2);
...
void Player::stopSleepInBed(bool_t a2, bool_t a3, bool_t a4) {
	...
	if(this->isSleeping()) {
		...
		if(Tile::isBed(level->getTile(v17.x, v17.y, v17.z))) {
