...
bool_t SignTile::use(Level* level, int32_t x, int32_t y, int32_t z, Player* player) {
	if(player && player->inventory) {
		ItemInstance* item = player->inventory->getSelected();
		if(item) {
			if(item->getId() == Item::dye_powder->itemID) {
				SignTileEntity* te = (SignTileEntity*)level->getTileEntity(x, y, z);
				if(te) {
					te->textColor = item->getAuxValue();
					return 1;
				}
			} else if (item->getId() == Item::paper->itemID) {
				SignTileEntity* te = (SignTileEntity*)level->getTileEntity(x, y, z);
				if(te) {
					te->textColor = 0;
					return 1;
