...
TextBox::TextBox(Minecraft* a2, const Options::Option* a3, const std::string& a4)
	...
	this->doneButton = new Touch::TButton(1, 0, 0, "Save", 0);
	this->doneButton->init(a2);
	this->doneButton->posX = (a2->currentScreen ? a2->currentScreen->width / 2 - this->doneButton->width / 2 : 0);
	this->field_44 = 16;
	this->validChars = "/\n\r\t";
	this->field_4C = 15;
	this->height = 18;
...
void TextBox::topRender(Minecraft* a2, int32_t a3, int32_t a4) {
	if(this->field_40) {
		...
		this->drawRect(5, 20, a2->currentScreen->width - 5, 37, -1, 1);
		std::string v10 = (getEpochTimeS() & 1) != 0 ? this->text + '_' : this->text;
		a2->font->draw(v10, 10.0, 25.0, -1, 0);
		this->doneButton->posX = a2->currentScreen->width / 2 - this->doneButton->width / 2;
		this->doneButton->posY = 45;
