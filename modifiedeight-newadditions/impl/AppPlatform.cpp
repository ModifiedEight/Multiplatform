...
TextureData AppPlatform::loadTexture(const std::string& a3, bool_t a4){
	...
	if (a3 == "terrain-atlas.tga") {
		if (Options::instance && Options::instance->classicTextures) {
			std::string altPath = "terrain-atlas.tga";
			AssetFile f = this->readAssetFile(altPath);
			if (!f.bytes) {
				altPath = "terrain.png";
				f = this->readAssetFile(altPath);
			}
		...
		if (g_terrainAtlasPixels != nullptr && g_terrainAtlasWidth > 0 && g_terrainAtlasHeight > 0) {
			...
			data.pixels = (uint8_t*)malloc(g_terrainAtlasWidth * g_terrainAtlasHeight * 4);
			memcpy(data.pixels, g_terrainAtlasPixels, g_terrainAtlasWidth * g_terrainAtlasHeight * 4);
			data.field_C = 0;
			data.field_10 = 0;
			data.lod = 0;
			data.field_18 = 0;
	...
	if (a3 == "items-opaque.png") {
		if (Options::instance && Options::instance->classicTextures) {
			std::string altPath = "items-opaque.png";
			AssetFile f = this->readAssetFile(altPath);
			if (!f.bytes) {
				altPath = "gui/items.png";
				f = this->readAssetFile(altPath);
			}
			if (f.bytes && f.length > 0) {
				int w, h, ch;
		...
		if (g_itemsAtlasPixels != nullptr && g_itemsAtlasWidth > 0 && g_itemsAtlasHeight > 0) {
			...
			memcpy(data.pixels, g_itemsAtlasPixels, g_itemsAtlasWidth * g_itemsAtlasHeight * 4);
			data.field_C = 0;
			data.field_10 = 0;
			data.lod = 0;
			data.field_18 = 0;
