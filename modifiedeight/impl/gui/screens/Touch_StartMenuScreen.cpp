...
void Touch::StartMenuScreen::render(int32_t a2, int32_t a3, float a4) {
  ...
  int32_t v10 =
      this->minecraft->texturesPtr->loadTexture("gui/title.png", 1, 0);
  if (v10) {
...
void Touch::StartMenuScreen::init() {
  ...
  v8.width = 8;
  v8.height = 0x43;
  this->settingsButtonMaybe->init(this->minecraft->texturesPtr, 32, 32, v12, v8,
  ...
  this->field_138 = "\x0fMojang AB, ModifiedEight Classic 1.6.0 by eqozqq";
...
void Touch::StartMenuScreen::setupPositions() {
  TextureData *data =
      this->minecraft->texturesPtr->loadAndGetTextureData("gui/title.png");
  if (data) {
  ...
  if (Options::instance && Options::instance->marketplace) {
    this->marketplaceButton.posY = this->playButton.posY + 35;
    this->playOnRealmsButton.posY = this->marketplaceButton.posY + 35;
  } else {
  ...
  this->buyButton.posX = (v16b - this->buyButton.width) / 2;
}
...
void Touch::StartMenuScreen::buttonClicked(struct Button *button) {
  ...
  if (button->buttonID == this->marketplaceButton.buttonID) {
