...
void ImageButton::render(struct Minecraft* mc, int32_t x, int32_t y){
	if(this->visible){
		...
		if(this->active){
			...
			this->drawCenteredString(mc->font, this->maybeTextOnButton, this->posX+this->width/2, this->posY+this->yOffset, 0xffa0a0a0);
		}
	}
}
void ImageButton::renderBg(struct Minecraft* mc, int32_t x, int32_t y){
}
