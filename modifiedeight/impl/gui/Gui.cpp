...
Gui::~Gui() { AppPlatform::_singleton->removeListener(this); }
...
long getMemoryUsageMB() {
  ...
  long pages = 0;
  if (statm >> pages >> pages) { // second value is RSS (Resident Set Size)
    return (pages * sysconf(_SC_PAGESIZE)) / (1024 * 1024);
  }
  return 0;
}
#else
long getMemoryUsageMB() { return 0; }
#endif
...
int32_t Gui::getNumSlots() { return this->slotsAmount; }
...
int32_t Gui::getSlotIdAt(int32_t a2, int32_t a3) {
  ...
  v5 =
      (int32_t)(float)((float)this->minecraftInst->field_1C * Gui::InvGuiScale);
  v6 =
      (int32_t)(float)((float)this->minecraftInst->field_20 * Gui::InvGuiScale);
...
void Gui::inventoryUpdated() { this->invUpdated = 1; }
  ...
  if (minecraftInst->level) {
    if (minecraftInst->player) {
      ...
      if (v18 > 0.0) {
        ...
        font->drawShadow(this->tipMessage,
      ...
      if (!this->minecraftInst->currentScreen) {
        ...
        if (!this->minecraftInst->options.hideGUI) {
          ...
          if (this->minecraftInst->options.debugScreen) {
            ...
            char_t dbgText1[128];
            sprintf(dbgText1, "ModifiedEight Classic 1.6.5pre4 (%d fps)", fps);
            font->drawShadow(dbgText1, 2.0f, startY, 0xFFFFFF);
            startY += 10.0f;

            char_t dbgText2[128];
            sprintf(dbgText2, "XYZ: %.3f / %.5f / %.3f",
                    this->minecraftInst->player->posX,
            ...
            char_t dbgText4[128];
            sprintf(dbgText4, "Chunk: %d %d %d",
                    ((int)floorf(this->minecraftInst->player->posX)) >> 4,
            ...
            Biome *biome = this->minecraftInst->level->getBiome(
                (int)floorf(this->minecraftInst->player->posX),
            ...
            if (this->minecraftInst->options.showCoordinates) {
              char_t coordsText[128];
              sprintf(coordsText, "X: %d Y: %d Z: %d",
                      (int)floorf(this->minecraftInst->player->posX),
                      (int)floorf(this->minecraftInst->player->posY -
                                  this->minecraftInst->player->ridingHeight),
                      (int)floorf(this->minecraftInst->player->posZ));
...
void Gui::renderBubbles() {
  ...
  if (this->minecraftInst->player->isUnderLiquid(Material::water)) {
    ...
    v3 = (int32_t)ceilf((float)((float)(relatedToBubbleRendering - 2) * 10.0) /
                        300.0);
    ...
    if (this->chatMessages.size() != 0) {
      ...
      while (v10 >= 0) {
        ...
        if (str->field_0 < str->field_4) {
          float v20 = (float)(1.0 - (float)((float)str->field_0 /
                                            (float)str->field_4)) *
                      10.0;
          if (v20 < 0)
            v20 = 0;
          ...
          if (v21 > 0) {
            if (v7 <= 10) {
              ++v16;
              int bgColor = v21 >> 1 << 24;
              int userBgColor = this->minecraftInst->options.chatBgColor;
              if (userBgColor == 1)
                bgColor = (v21 >> 1 << 24) | 0x880000;
              else if (userBgColor == 2)
                bgColor = (v21 >> 1 << 24) | 0x008800;
              else if (userBgColor == 3)
                bgColor = (v21 >> 1 << 24) | 0x000088;
              else if (userBgColor == 4)
                bgColor = 0;

              float lineY = (float)(9 * (v16 - 1) + chatBaseY);
              this->fill(2.0f, lineY - 1.0f, (float)this->field_18 + 2.0f,
                         lineY + 8.0f, bgColor);
              ...
              if (str->field_8[0] == '/') {
                ...
                int userColor = this->minecraftInst->options.chatColor;
                if (userColor == 1)
                  textColor = 0xAAAAAA; // Gray
                else if (userColor == 2)
                  textColor = 0xFFFF55; // Yellow
                else if (userColor == 3)
                  textColor = 0x55FF55; // Green
                else if (userColor == 4)
                  textColor = 0xFF5555; // Red
                else if (userColor == 5)
                  textColor = 0x5555FF; // Blue
                else if (userColor == 6)
                  textColor = 0xFFAA00; // Gold
                else if (userColor == 7)
                  textColor = 0x55FFFF; // Aqua
                else if (userColor == 8)
  ...
  if (this->field_A00 < 1.0) {
    ...
    if (sel) {
      ...
      if (this->field_A00 <= 0.75) {
        ...
        v11 = (int32_t)(float)(this->cubeSmoothStep(
                                   (float)(0.25 -
                                           (float)(this->field_A00 - 0.75)) *
                                       4.0,
                                   0.0, 1.0) *
                               255.0);
        if (!v11) {
      ...
      a3->drawShadow(sel->getName(), (float)(a2 / 2 - v10 / 2),
                     (float)(a4 - 22), (v11 << 24) + 0xFFFFFF);
    }
  }
}
void Gui::renderProgressIndicator(bool_t a2, int32_t a3, int32_t a4, float a5) {
  ...
  if (!a2 ||
      (minecraftInst = this->minecraftInst, minecraftInst->options.useJoypad)) {
  LABEL_8:
    this->minecraftInst->texturesPtr->loadAndBindTexture("gui/icons.png");
...
LABEL_21:
  ...
  glTranslatef(Gui::InvGuiScale * this->minecraftInst->inputHolder->mouseX,
               Gui::InvGuiScale * this->minecraftInst->inputHolder->mouseY,
               0.0);
  this->outerBreakRingMesh.render();
...
void Gui::renderSlotText(const struct ItemInstance *a2, float a3, float a4,
                         bool_t a5, bool_t a6) {
  int count;     // r1
  int v11;       // r3
  Font *font;    // r0
...
void Gui::renderToolBar(float a2, float a3) {
  ...
  if (this->field_A84 >= 0) {
    if (inventory->getItem(this->field_A84)) {
      ...
      this->fill(v35,
                 v8 - 3 -
                     (int32_t)(float)((float)((float)(v36 + a2) * 17.0) / 40.0),
                 v35 + 16, v8 - 3, 0x8000FF00);
    }
  }

  this->blit(sp - 1 + 20 * inventory->selectedSlot, sp2 - 1, 0, 22, 24, 22, 0,
             0);
  glColor4f(1.0, 1.0, 1.0, 1.0);
  if (this->field_A18 >= 0) {
    ...
    if (v14 <= 0.2) {
      ...
      this->fill(v15 + 2, v8 - 19, v15 + 18, v8 - 3,
                 ((int32_t)(float)(81.0 - (float)(v16 * 80.0)) << 24) +
                     0xFFFFFF);
    } else {
      this->field_A18 = -1;
    }
  ...
  this->blit(10 * NumSlots + v7 / 2 - 16, v23, 228, 248, 14, 4, 28, 8);
  {
    ...
    while (v24 < this->getNumSlots() - 1) {
      ...
      ItemRenderer::renderGuiItemDecorations(
          this->minecraftInst->player->inventory->getItem(v26), v25, (float)v9);
      v25 = v25 + 20.0;
    }
    Tesselator::instance.endOverrideAndDraw();
  ...
  glPushMatrix();
  glScalef(Gui::InvGuiScale + Gui::InvGuiScale,
           Gui::InvGuiScale + Gui::InvGuiScale, 1.0);
  v29 = Gui::GuiScale;
  Tesselator::instance.beginOverride();
  if (this->minecraftInst->gameMode->isSurvivalType()) {
    v31 = 0;
    ...
    while (v31 < this->getNumSlots() - 1) {
      ...
      if (v34) {
        if (v34->count >= 0) {
          this->renderSlotText(v34, v32 * v12, (float)(v32 * (float)v9) + 1.0,
                               1, 1);
        }
      }
      v12 = v12 + 20.0;
...
void Gui::resetItemNameOverlay() { this->field_A00 = 0; }
...
void Gui::setScissorRect(const IntRectangle &a2) {
  glScissor((uint32_t)(float)(Gui::GuiScale * (float)a2.minX),
            this->minecraftInst->field_20 -
                (uint32_t)(float)(Gui::GuiScale * (float)(a2.height + a2.minY)),
            (uint32_t)(float)(Gui::GuiScale * (float)a2.width),
            (uint32_t)(float)(Gui::GuiScale * (float)a2.height));
}
