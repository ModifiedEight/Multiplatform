...
BiomeSource::BiomeSource(struct Level* a2)
	...
	this->detailNoises = new float[256];
}
...
void BiomeSource::resizeBuffers(int32_t newCapacity) {
	if (newCapacity <= this->noisesCapacity) return;
	...
	this->noisesCapacity = newCapacity;
	this->rainfallNoises = new float[newCapacity];
	this->temperatureNoises = new float[newCapacity];
	this->detailNoises = new float[newCapacity];
	this->biomes = new Biome*[newCapacity];
}
