...
NewRandomLevelSource::NewRandomLevelSource(struct Level *a2, int32_t a3,
                                           int32_t a4, bool a5)
    : random(a3), octave16noise_1(&this->random, 16),
      octave16noise_2(&this->random, 16), octave8noise_1(&this->random, 8),
      octave4noise_1(&this->random, 4), octave4noise_2(&this->random, 4),
      octave10noise_1(&this->random, 10), octave16noise_3(&this->random, 16),
...
void NewRandomLevelSource::buildSurfaces(int32_t a2, int32_t a3, uint8_t *a4,
                                         struct Biome **a5) {
  float v5;
  float v8;
  int32_t v9;
  ...
  v5 = (float)(16 * a2);
  v8 = (float)(16 * a3);
  this->octave4noise_1.getRegion(this->field_72D4, v5, v8, 0.0, 16, 16, 1,
                                 0.03125, 0.03125, 1.0);
  this->octave4noise_1.getRegion(this->field_76D4, v5, 109.01, v8, 16, 1, 16,
                                 0.03125, 1.0, 0.03125);
  this->octave4noise_2.getRegion(this->field_7AD4, v5, v8, 0.0, 16, 16, 1,
                                 0.0625, 0.0625, 0.0625);

  for (int blockX = 0; blockX < 16; ++blockX) {
    for (int blockZ = 0; blockZ < 16; ++blockZ) {
      Biome *biome = a5[blockX + (blockZ * 16)];
      bool z = this->field_72D4[blockX + (blockZ * 16)] +
                   (this->random.nextFloat() * 0.2f) >
               0.0f;
      bool z2 = this->field_76D4[blockX + (blockZ * 16)] +
                    (this->random.nextFloat() * 0.2f) >
                3.0f;
      int nextFloat = (int)((this->field_7AD4[blockX + (blockZ * 16)] / 3.0f) +
                            3.0f + (this->random.nextFloat() * 0.25f));
      int i = -1;
...
void NewRandomLevelSource::calcWaterDepths(struct ChunkSource *a2, int32_t a3,
                                           int32_t a4) {
  Level *level;
  int32_t x;
  int32_t z;
  ...
  do {
    ...
    do {
      v8 = z + 7;
      if (level->getHeightmap(x - 1, z + 7) <= 0 &&
          (level->getHeightmap(xsub2, z + 7) > 0 ||
           level->getHeightmap(x, z + 7) > 0 ||
           level->getHeightmap(x - 1, z + 6) > 0 ||
           level->getHeightmap(x - 1, z + 8) > 0) &&
          (level->getTile(xsub2, y, z + 7) == Tile::calmWater->blockID &&
               level->getData(xsub2, y, z + 7) <= 6 ||
           level->getTile(x, y, z + 7) == Tile::calmWater->blockID &&
               level->getData(x, y, z + 7) <= 6 ||
           level->getTile(x - 1, y, z + 6) == Tile::calmWater->blockID &&
               level->getData(x - 1, y, z + 6) <= 6 ||
           level->getTile(x - 1, y, z + 8) == Tile::calmWater->blockID &&
               level->getData(x - 1, y, z + 8) <= 6)) {
        for (i = -5; i != 6; ++i) {
          v14 = -5;
          v22 = abs(i);
          v18 = x - 1 + i;
          do {
            ...
            if ((int32_t)(v22 + v9) <= 5) {
              v16 = v8 + v14;
              if (level->getTile(v18, y, v8 + v14) ==
                  Tile::calmWater->blockID) {
        ...
        v12 = 0;
        level->setTileAndDataNoUpdate(x - 1, y, z + 7, Tile::calmWater->blockID,
                                      7);
        while (v12 < y) {
          v13 = v12++;
          level->setTileAndDataNoUpdate(x - 1, v13, z + 7,
                                        Tile::calmWater->blockID, 8);
        }
      }
      ++z;
      ++v15;
...
float *NewRandomLevelSource::getHeights(float *heights, int32_t chunkX,
                                        int32_t chunkY, int32_t chunkZ,
  ...
  this->biomeNoises = this->octave10noise_1.getRegion(
      this->biomeNoises, chunkX, chunkZ, scaleX, scaleZ, 1.121, 1.121, 0.5);
  v14 = (float)chunkY;
  this->depthNoises = this->octave16noise_3.getRegion(
      this->depthNoises, chunkX, chunkZ, scaleX, scaleZ, 200.0, 200.0, 0.5);
  this->interpolationNoises = this->octave8noise_1.getRegion(
      this->interpolationNoises, (float)chunkX, v14, (float)chunkZ, scaleX,
      scaleY, scaleZ, 8.5552, 4.2776, 8.5552);
  this->upperInterpolationNoises = this->octave16noise_1.getRegion(
      this->upperInterpolationNoises, (float)chunkX, v14, (float)chunkZ, scaleX,
      scaleY, scaleZ, 684.41, 684.41, 684.41);
  this->lowerInterpolationNoises = this->octave16noise_2.getRegion(
      this->lowerInterpolationNoises, (float)chunkX, v14, (float)chunkZ, scaleX,
      scaleY, scaleZ, 684.41, 684.41, 684.41);
  ...
  while (v17 < scaleX) {
    ...
    for (i = 0; i < scaleZ; ++i) {
      ...
      v25 = (float)((float)(this->biomeNoises[v23] + 256.0) * 0.0019531) *
            (float)(1.0 - (float)((float)(v22 * v22) * (float)(v22 * v22)));
      if (v25 > 1.0) {
        v25 = 1.0;
      }
      if (v24 < 0.0) {
      ...
      if (this->level->getLevelData()->getGeneratorVersion() >= 3) {
        if (v28 > 0)
          v28 *= 2.5f;
      }
      v29 = v20;
      v30 = 0;
      v31 = v25 + 0.5;
      v32 = (float)((float)((float)(v28 * (float)scaleY) * 0.0625) * 4.0) +
            (float)((float)scaleY * 0.5);
      while (v30 < scaleY) {
        v33 = (float)((float)((float)v30 - v32) * 12.0) / v31;
        v34 =
            (float)((float)(this->interpolationNoises[v29] / 10.0) + 1.0) * 0.5;
        if (v33 < 0.0) {
          v33 = v33 * 4.0;
        }
        ...
        if (v34 >= 0.0) {
          if (v34 > 1.0) {
            ...
            v35 = v35 +
                  (float)((float)((float)(this->lowerInterpolationNoises[v29] *
                                          0.0019531) -
                                  v35) *
                          v34);
          }
        ...
        if (scaleY - 3 <= v30) {
          v36 =
              (float)((float)((float)(4 - scaleY + v30) / 3.0) * -10.0) +
              (float)(v36 *
                      (float)(1.0 - (float)((float)(4 - scaleY + v30) / 3.0)));
        }
        ++v30;
        v37 = &heights[v29++];
        *v37 = v36;
...
void NewRandomLevelSource::prepareHeights(int32_t a2, int32_t a3, uint8_t *a4,
                                          void *a5, float *a6) {
  ...
  this->field_72D0 = heights =
      this->getHeights(this->field_72D0, 4 * a2, 0, 4 * a3, 5, 17, 5);
  v34 = 0;
  while (2) {
    v44 = 85 * v34;
    v45 = 85 * v34 + 340;
...
bool_t NewRandomLevelSource::hasChunk(int32_t x, int32_t z) { return 1; }
struct LevelChunk *NewRandomLevelSource::getChunk(int32_t chunkX,
                                                  int32_t chunkZ) {
  uint32_t v5;
  int32_t v6;
  int32_t v9;
  ...
  v16 = this->level->getBiomeSource()->getBiomeBlock(16 * chunkX, 16 * chunkZ,
                                                     16, 16);
  if (this->level->getLevelData()->getGeneratorVersion() == 2) {
    ...
    for (int x = 0; x < 16; ++x) {
      for (int z = 0; z < 16; ++z) {
        int base = (x << 11) | (z << 7);
        chunkData[base + 0] = Tile::unbreakable->blockID;
        chunkData[base + 1] = Tile::dirt->blockID;
        chunkData[base + 2] = Tile::dirt->blockID;
        chunkData[base + 3] = Tile::grass->blockID;
        for (int y = 4; y < 128; ++y) {
          chunkData[base + y] = 0;
        }
      }
...
void NewRandomLevelSource::postProcess(struct ChunkSource *a2, int32_t chunkX,
                                       int32_t chunkZ) {
  ...
  this->random.setSeed((v12 + (2 * (v11 >> 2) + 1) * chunkX) ^
                       this->level->getSeed());
  ...
  if (this->level->getLevelData()->getGeneratorVersion() != 2) {
    ...
    float v = this->treeNoise.getValue((float)chunkXStart * 0.5,
                                       (float)chunkZStart * 0.5);
    int v67 = (int)(float)((float)((float)((float)(a8->nextFloat() * 4.0) +
                                           (float)(v * 0.125)) +
                                   4.0) /
                           3.0);
    int v68 = v67;
    ...
    if (biomeAtChunk == Biome::birchForest && (a8->genrand_int32() % 5) == 0) {
      ...
      int32_t hm = this->level->getHeightmap(xx, zz);
      if (hm > 0 &&
          this->level->getTile(xx, hm - 1, zz) == Tile::grass->blockID) {
        int len = 3 + (a8->genrand_int32() % 3);
        int dx = (a8->genrand_int32() % 2 == 0) ? 1 : 0;
        int dz = 1 - dx;
        int meta = (dx == 1) ? 6 : 10;
        for (int i = 0; i < len; ++i) {
          if (this->level->isEmptyTile(xx + i * dx, hm, zz + i * dz)) {
            this->level->setTileAndDataNoUpdate(xx + i * dx, hm, zz + i * dz,
                                                Tile::treeTrunk->blockID, meta);
          }
        }
      }
    }
    if (biomeAtChunk == Biome::jungle) {
      for (int i = 0; i < 4; ++i) {
        ...
        int32_t hm = this->level->getHeightmap(xx, zz);
        if (hm > 0 &&
            this->level->getTile(xx, hm, zz) == 0 &&
            this->level->getTile(xx, hm - 1, zz) == Tile::grass->blockID) {
          this->level->setTileAndDataNoUpdate(xx, hm, zz,
                                              Tile::treeTrunk->blockID, 3);
          this->level->setTileAndDataNoUpdate(xx, hm + 1, zz,
                                              Tile::leaves->blockID, 3);
          this->level->setTileAndDataNoUpdate(xx + 1, hm, zz,
                                              Tile::leaves->blockID, 3);
          this->level->setTileAndDataNoUpdate(xx - 1, hm, zz,
                                              Tile::leaves->blockID, 3);
          this->level->setTileAndDataNoUpdate(xx, hm, zz + 1,
                                              Tile::leaves->blockID, 3);
          this->level->setTileAndDataNoUpdate(xx, hm, zz - 1,
                                              Tile::leaves->blockID, 3);
        }
      ...
      for (int i = 0; i < 3; ++i) {
        ...
        if (hm > 0 &&
            this->level->getTile(xx, hm - 1, zz) == Tile::grass->blockID) {
          this->level->setTileAndDataNoUpdate(xx, hm, zz, Tile::melon->blockID,
                                              0);
        }
      }
    }
    if (biomeAtChunk == Biome::mountain) {
      Random mr(chunkX * 341873128 + chunkZ * 1328979875LL);
    }

    int grassCount = 0;
    if (biomeAtChunk == Biome::plains || biomeAtChunk == Biome::forest ||
        biomeAtChunk == Biome::birchForest || biomeAtChunk == Biome::jungle ||
        biomeAtChunk == Biome::savanna) {
      grassCount = 1;
    ...
    for (int32_t v76 = 0; v76 < 2; ++v76) {
      ...
      f.place(this->level, a8, chunkXStart + (v77 & 0xF) + 8, v78 & 0x7F,
              chunkZStart + (v79 & 0xF) + 8);
    }
    if ((a8->genrand_int32() & 1) == 0) {
      ...
      f.place(this->level, a8, chunkXStart + (v80 & 0xF) + 8, v81 & 0x7F,
              chunkZStart + (v82 & 0xF) + 8);
    }
    if ((a8->genrand_int32() & 3) == 0) {
      ...
      f.place(this->level, a8, chunkXStart + (v84 & 0xF) + 8, v85 & 0x7F,
              chunkZStart + (v86 & 0xF) + 8);
    }
    if ((a8->genrand_int32() & 7) == 0) {
      ...
      f.place(this->level, a8, chunkXStart + (v89 & 0xF) + 8, v90 & 0x7F,
              chunkZStart + (v91 & 0xF) + 8);
    }
    for (int32_t v95 = 0; v95 < 10; ++v95) {
      ...
      ReedsFeature f;
      f.place(this->level, a8, chunkXStart + (v96 & 0xF) + 8, v97 & 0x7F,
              chunkZStart + (v98 & 0xF) + 8);
    }
    int32_t cactiCnt;
    if (biomeAtChunk == Biome::desert) {
      cactiCnt = 5;
    ...
    for (int32_t v95 = 0; v95 < cactiCnt; ++v95) {
      ...
      CactusFeature f;
      f.place(this->level, a8, chunkXStart + (v100 & 0xF) + 8, v101 & 0x7F,
              chunkZStart + (v102 & 0xF) + 8);
    }
    for (int32_t v103 = 0; v103 < 50; ++v103) {
      ...
      f.place(this->level, a8, (v104 & 0xF) + chunkXStart + 8,
              v135 % (v137 % 0x78u + 8), chunkZStart + (v105 & 0xF) + 8);
    }
    for (int32_t v103 = 0; v103 < 20; ++v103) {
      ...
      f.place(this->level, a8, (v108 & 0xF) + chunkXStart + 8,
              v109 % (v136 % (v138 % 0x70u + 8) + 8),
              chunkZStart + (v110 & 0xF) + 8);
    }
  }
  if (this->field_72CC) {
    MobSpawner::postProcessSpawnMobs(this->level, biomeAtChunk, chunkXStart + 8,
                                     chunkZStart + 8, 16, 16, a8);
  }

  if (this->level->getLevelData()->getGeneratorVersion() != 2) {
    int v113 = chunkXStart + 8;
    float *v114 = this->level->getBiomeSource()->getTemperatureBlock(
        chunkXStart + 8, chunkZStart + 8, 16, 16);
    while (chunkXStart + 23 >= v113) {
      ...
      while (chunkZStart + 23 >= v119) {
        ...
        float v116 = *v142++;
        int v117 = topy;
        if ((float)(v116 - (float)((float)((float)(topy - 64) * 0.015625) *
                                   0.3)) < 0.5 &&
            (unsigned int)(topy - 1) <= 0x7E &&
            this->level->isEmptyTile(v113, topy, v119)) {
          Material *v118 = this->level->getMaterial(v113, v117 - 1, v119);
          if (v118->blocksMotion()) {
            if (Material::ice !=
                this->level->getMaterial(v113, v117 - 1, v119)) {
...
bool_t NewRandomLevelSource::tick() { return 0; }
bool_t NewRandomLevelSource::shouldSave() { return 1; }
std::vector<Biome::MobSpawnerData>
NewRandomLevelSource::getMobsAt(const struct MobCategory &a3, int32_t a4,
