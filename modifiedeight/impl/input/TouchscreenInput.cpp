...
void TouchscreenInput::rebuild() {
	...
	if(!options->hideGUI) {
		...
		if(!this->minecraft->currentScreen && this->cameraButton) {
			...
			int32_t bgColor = this->isButtonDown(107) ? 0x55000000 : 0x33000000;
			int32_t borderColor = this->isButtonDown(107) ? 0x99FFFFFF : 0x66FFFFFF;

			this->field_14.fill(minX + 1.0f, minY + 1.0f, maxX - 1.0f, maxY - 1.0f, bgColor);
			this->field_14.fill(minX + 1.0f, minY, maxX - 1.0f, minY + 1.0f, borderColor);
			this->field_14.fill(minX + 1.0f, maxY - 1.0f, maxX - 1.0f, maxY, borderColor);
			this->field_14.fill(minX, minY + 1.0f, minX + 1.0f, maxY - 1.0f, borderColor);
			this->field_14.fill(maxX - 1.0f, minY + 1.0f, maxX, maxY - 1.0f, borderColor);

			this->field_14.fill(cx - 3.0f, cy - 4.0f, cx, cy - 2.0f, 0x77000000);
			this->field_14.fill(cx - 5.0f, cy - 2.0f, cx + 7.0f, cy + 5.0f, 0x77000000);

			this->field_14.fill(cx - 4.0f, cy - 5.0f, cx - 1.0f, cy - 3.0f, 0xFFFFFFFF);
			this->field_14.fill(cx - 6.0f, cy - 3.0f, cx + 6.0f, cy + 4.0f, 0xFFFFFFFF);
			this->field_14.fill(cx - 3.0f, cy - 2.0f, cx + 3.0f, cy + 3.0f, 0xFF888888);
			this->field_14.fill(cx - 2.0f, cy - 1.0f, cx + 2.0f, cy + 2.0f, 0xFF2266AA);
			this->field_14.fill(cx - 1.0f, cy, cx + 1.0f, cy + 1.0f, 0xFF55AFFF);
			this->field_14.fill(cx, cy - 1.0f, cx + 1.0f, cy, 0xFFFFFFFF);
		}
	}
}
