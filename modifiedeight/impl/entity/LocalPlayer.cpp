...
LocalPlayer::LocalPlayer(Minecraft *a2, Level *a3, User *a4, int32_t a5,
                         bool_t a6)
    : Player(a3, a6), inventoryMenu(this->inventory) {
  this->moveInput = 0;
...
void LocalPlayer::calculateFlight(float a2, float a3, float a4) {
  ...
  this->field_D90 = this->field_D9C.getNewDeltaValue(
                        a2 * v6, minecraft->options.sensitity * 0.35) *
                    10.0;
  this->field_D94 = this->field_DA8.getNewDeltaValue(
                        v9, this->minecraft->options.sensitity * 0.35) *
                    10.0;
  this->field_D98 = this->field_DB4.getNewDeltaValue(
                        a4 * v6, this->minecraft->options.sensitity * 0.35) *
                    10.0;
}
float LocalPlayer::getFieldOfViewModifier() {
  ...
  v4 = v1 *
       (float)((float)((float)((float)(v3 * this->getWalkingSpeedModifier()) /
                               0.1) +
                       1.0) *
               0.5);
...
bool_t LocalPlayer::isSolidTile(int32_t a2, int32_t a3, int32_t a4) {
  ...
  if (!Tile::tiles[v4]->isSolidRender() && Tile::tiles[v4]->getAABB(this->level, a2, a3, a4) == nullptr) {
    return 0;
  }
...
void LocalPlayer::respawn() { this->minecraft->respawnPlayer(); }
void LocalPlayer::sendPosition() {
  MovePlayerPacket v18(this->entityId, this->posX,
                       this->posY - this->ridingHeight, this->posZ, this->pitch,
  ...
  if (this->isRiding()) {
    PlayerInputPacket v17(this->moveForward, this->moveStrafe, this->isJumping,
                          this->isSneaking());
...
void LocalPlayer::move(float a2, float a3, float a4) {
  ...
  if (this == minecraft->player && minecraft->options.enableNoclipMaybe) {
    ...
    v12 = this->posX;
    v13 = this->posZ;
    if (this->field_DC0 <= 0 && this->minecraft->options.autoJump &&
        !this->isRiding()) {
      v16 = Mth::floor(posX + posX);
      if (v16 != Mth::floor(v12 + v12) ||
          (v14 = Mth::floor(posZ + posZ), v14 != Mth::floor(v13 + v13))) {
...
void LocalPlayer::tick() {
  ...
  if (this->minecraft->isOnline()) {
    if (this->isRiding() || fabsf(this->posX - this->field_BC8) > 0.1 ||
        ...
        fabsf(this->field_BD8 - this->yaw) > 1.0 ||
        fabsf(this->field_DCC - this->headYaw) > 1.0) {
      this->sendPosition();
    }
    if (this->level->isClientMaybe) {
    ...
    if ((this->currentItemID != id) || (this->currentItemMeta != auxv)) {
      ...
      int32_t ls =
          this->inventory->getLinkedSlot(this->inventory->selectedSlot);
...
void LocalPlayer::ride(Entity *a2) {
  ...
  if (!level->isClientMaybe) {
    SetEntityLinkPacket v9(0, this->entityId,
                           this->ridingAt ? this->ridingAt->entityId
                                          : 0); // [sp+8h] [bp-30h] BYREF
    this->minecraft->rakNetInstance->send(v9);
  }
}
bool_t LocalPlayer::isSneaking() { return this->moveInput->sneakingMaybe; }
...
bool_t LocalPlayer::isLocalPlayer() { return 1; }
...
void LocalPlayer::refreshContainer(BaseContainerMenu *,
                                   const std::vector<ItemInstance> &) {}
void LocalPlayer::slotChanged(BaseContainerMenu *a2, int32_t a3,
                              const ItemInstance &a4, bool_t a5) {
...
void LocalPlayer::setSprinting(bool_t s) {
  if (s && !this->minecraft->options.sprintEnabled) {
    this->isSprinting = 0;
  } else {
    this->isSprinting = s;
  }
